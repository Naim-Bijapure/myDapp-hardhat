...
contract RollTheDice is Context, Ownable, AccessControl, VRFConsumerBaseV2 {
    ...
    event Prediction(uint256 gameCounter, uint8 round, address player, uint8 prediction);
     ...
     */
    function setMaxTargetScore(uint256 _maxTargetScore) external onlyRole(ADMIN_ROLE) {
     ...
     * - the caller must have {ADMIN_ROLE}
     */
    function getGameCounter() external view onlyRole(ADMIN_ROLE) returns (uint256) {
        return gameCounter;
    }
    ...
    function createGame(uint8 targetScore, uint256 blind) public payable {
        ...
        require(msg.value >= blind, "msg.value should be equal to or greater than blind");
    ...
    function bet(uint256 gameId) public payable {
        ...
        require(msg.value >= games[gameId].blind, "Bet should be greater than blind");
        require(games[gameId].bets[_msgSender()] == 0, "Already betted");
        require(games[gameId].state == GAME_STATE_BETTING, "Betting has already ended");
        require(games[gameId].players.length < 255, "Cannot have more than 255 players");
    ...
    function predictAndRoll(uint256 gameId, uint8 prediction) external {
        ...
        if (round == 0) {
            require(players.length > 1, "At least two players are required to play");
            require(state == GAME_STATE_BETTING, "Invalid game state");
        } else {
            require(state == GAME_STATE_PLAYING, "Invalid game state");
        }
        require(prediction >= 1 && prediction <= 6, "Prediction should be between 1 and 6");
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
        ...
        require(games[gameId].state == GAME_STATE_WAITING, "Invalid game state");
     ...
     */
    function _convertRandomWordsToFace(uint256[] memory randomWords) internal pure returns (uint8) {
    ...
    function _handleOutput(uint256 gameId, uint8 face) internal {
        ...
        if (face == prediction) {
            ...
            nextTurn = _getNextTurn(turn, uint8(games[gameId].players.length - 1));
        }
    ...
    function _getNextTurn(uint8 turn, uint8 maxTurn) internal pure returns (uint8) {
        unchecked {
