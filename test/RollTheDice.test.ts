...
import { RollTheDice } from "../client/src/typechain";
...
describe.skip("RollTheDice", () => {
    ...
    async function deployContract(
        vrfCoordinatorContract: "MockVRFCoordinator" | "MockVRFCoordinatorUnfulfillable" = "MockVRFCoordinator"
    ) {
        const contractFactory = await ethers.getContractFactory("RollTheDice");

        const vrfCoordFactory = await ethers.getContractFactory(vrfCoordinatorContract);
        const mockVrfCoordinator = await vrfCoordFactory.connect(owner).deploy();

        return await contractFactory.connect(owner).deploy(mockVrfCoordinator.address, MOCK_LINK, MOCK_SUBSCRIPTION_ID);
    }

    async function createGame(caller: string | SignerWithAddress, targetScore: number, blind: number) {
    ...
    describe("setMaxTargetScore", () => {
        ...
        it("should update maxTargetScore when caller is admin", async () => {
            const tx = await contract.connect(owner).setMaxTargetScore(newMaxTargetScore);
            await tx.wait();
        ...
        it("should revert when caller is not admin", async () => {
            ...
            await expect(contract.connect(caller).setMaxTargetScore(newMaxTargetScore)).to.be.reverted;
        });
    });
    ...
    describe("createGame", () => {
        ...
        it("should revert when blind is zero", async () => {
            await expect(contract.connect(caller).createGame(20, 0)).to.be.revertedWith(
                "Blind should be greater than zero"
            );
        });
        ...
        it("should revert when msg.value is smaller than blind", async () => {
            ...
            ).to.be.revertedWith("msg.value should be equal to or greater than blind");
        });
    ...
    describe("predictAndRoll", () => {
        ...
        context("when there is only one player", () => {
            it("should revert", async () => {
                await expect(contract.connect(gameCreator).predictAndRoll(gameId, 5)).to.be.revertedWith(
        ...
        context("when there are multiple players", () => {
            ...
            it("should revert when it is not caller's turn", async () => {
                await expect(contract.connect(player2).predictAndRoll(gameId, 5)).to.revertedWith("It's not your turn");
            });
            ...
            it("should revert when round is 0 and game state is not GAME_STATE_BETTING", async () => {
                ...
                await (await contract.connect(player1).predictAndRoll(gameId, 5)).wait();
                await expect(contract.connect(player1).predictAndRoll(gameId, 6)).to.be.revertedWith(
                    "Invalid game state"
                );
            });

            it("should revert when prediction is not between 1 and 6", async () => {
                await expect(contract.connect(player1).predictAndRoll(gameId, 0)).to.be.revertedWith(
                    "Prediction should be between 1 and 6"
                );

                await expect(contract.connect(player1).predictAndRoll(gameId, 7)).to.be.revertedWith(
                    "Prediction should be between 1 and 6"
                );
            });
        });
    ...
    context("when there are two players", () => {
        ...
        it("should switch turn when prediction is not correct", async () => {
            ...
            expect(contract.connect(player1).predictAndRoll(0, 3)).to.be.revertedWith("It's not your turn");
